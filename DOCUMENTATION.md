...
The hosted Relay is a separate optional service and is not included in the Marketplace license. It is free to try on the public demo. A live site verifies its authorized sender first, then activates Relay through Stripe Checkout inside Secretary for **USD 49/year per site**. Relay credentials and the site's readable address are not issued until Stripe confirms an active subscription. Control Panel chat and a private Postmark setup remain available without a Relay subscription.
