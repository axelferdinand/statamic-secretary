...
**Secretary Relay** is the quickest setup. Verify an existing permitted Statamic user and receive a site-specific address such as `example.com@statamic.no`. No mailbox, Postmark account, webhook, or queue worker is required. Try the email workflow on the public demo; using Relay on a live site is a separate **USD 49/year** subscription activated through secure Stripe Checkout inside Secretary.
