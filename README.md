...
One public mailbox must not be connected directly to multiple sites. The addon includes a disabled-by-default signed receiver and reply client for the optional shared `secretary@statamic.no` service. Requests are bound to one opaque installation ID, one route token, an independent 256-bit secret, a short timestamp window, the exact body digest, and a single-use nonce; replies remain bound to the same route and conversation. This repository also contains a framework-independent hosted-relay core under `relay/`, deliberately excluded from the Composer addon archive. It proves exact routing, ambiguity rejection, Postmark normalization, signed site delivery, idempotent reply binding, encrypted operator controls, two-phase signing-secret rotation, and route rotation that preserves existing threads without accepting new mail on retired aliases, but it is not a deployed service. The contract and remaining production work are documented in [docs/shared-address-relay.md](docs/shared-address-relay.md). Until that hosted service is deployed and independently verified, every installation must use a site-controlled address and Postmark server.
